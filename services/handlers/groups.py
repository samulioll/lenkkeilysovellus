...
def group_leaderboard(stat):
    options = {1: "total",
               2: "walked",
               3: "ran",
               4: "cycled",
               5: "time"}
    query_firs ="""SELECT name AS group, id AS group_id, SUM(total) AS total, SUM(walked) AS walked,
                   ...
                   GROUP BY name, id"""
    order = query_firs + " ORDER BY " + str(options[stat]) + " DESC"
    sql = text(order)
