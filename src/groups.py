...
def user_groups_overview():
    sql = text("SELECT G.name, G.id FROM groups G, groupmembers M WHERE G.id=M.group_id AND M.user_id=:user_id AND M.visible=TRUE")
...
def group_overview(form):
    sql = text("SELECT U.username FROM users U, groupmembers G WHERE U.id=G.user_id and G.group_id=:group_id AND G.visible=TRUE")
    result = db.session.execute(sql, {"group_id":form["group_id"]})
    return result.fetchall()
